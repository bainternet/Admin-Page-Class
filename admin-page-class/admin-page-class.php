 ...
 */
  ...
  class BF_Admin_Page_Class {
    ...
    /**
     * $inGroup flag for grouping post meta fields
     * @var boolean
     * @since 1.2.4
     * @access public
     */
    ...
    public function __construct($args) {
      if (!is_array($args) || !isset($args['data_type']))
        return;
      if (!in_array($args['data_type'], array('options','post_meta','tax_meta','user_meta')))
        $args['data_type'] = 'options';
      else{
        $this->args = $args;
        $this->data_type = $args['data_type'];
        call_user_func ( array( $this, 'init_'.$args['data_type'] ),$args);
        $this->init_shared();
      }
    }
    ...
    /**
     * init_options creates an admin page
     * @param  array $args 
     * @since 1.2.4
     ...
     * @return void
     */
    public function init_options($args){
      if(is_array($args)) {
      ...
      if(!is_array($args['menu'])) {
        if(is_object($args['menu'])) { //options data object
          ...
          if (in_array($args['menu'], array_keys($top))) //default menus
            $this->Top_Slug = $top[$args['menu']]; 
          elseif(post_type_exists($args['menu'])) //cutstom post type menu
    ...
    public function init_post_meta($args){
      //scripts and help tabs
      add_action('load-post.php', array($this,'Load_metabox_hooker'));
      add_action('load-post-new.php', array($this,'Load_metabox_hooker'));
      // Add metaboxes
    ...
    public function add_post_meta_box() {
      ...
      foreach ( $this->args['pages'] as $page ) {
        add_meta_box( $this->args['id'], $this->args['title'], array( &$this, 'show_post_meta' ), $page, $this->args['context'], $this->args['priority'] );
      }
    }

    /**
     * show_post_meta creates metabox html
     * @since 1.2.4
     * @access public
     * @return void
     */
     ...
     * @access public
     */
    public function Load_metabox_hooker(){
      global $typenow;
      if (!in_array($typenow, $this->args['pages']))
        return;
      //help tabs
    ...
    public function save_post_meta( $post_id ) {
      ...
      foreach ( $this->_fields as $field ) {
        ...
        $validationClass = apply_filters('apc_validattion_class_name', 'Admin_Page_Class_Validate',&$this);
        ...
        if ($type != "paragraph"){
          ...
          $save_func = 'save_post_meta_field_' . $type;
          if ( method_exists( $this, $save_func ) ) {
            call_user_func( array( &$this, 'save_field_' . $type ), $post_id, $field, $old, $new );
          } else {
            $this->save_post_meta_field( $post_id, $field, $old, $new );
          }
        }
    ...
    /**
     * Common function for saving post meta fields.
     *
     * @param string $post_id 
     * @param string $field 
     * @param string $old 
     ...
     * @access public
     */
    public function save_post_meta_field( $post_id, $field, $old, $new ) {
      $name = $field['id'];
    ...
    /**
     * function for post_meta saving image field.
     *
     * @param string $post_id 
     * @param string $field 
     * @param string $old 
     ...
     * @access public
     */
    public function save_post_meta_field_image( $post_id, $field, $old, $new ) {
      $name = $field['id'];
      delete_post_meta( $post_id, $name );
     ...
     * @access public 
     */
    public function save_post_meta_field_wysiwyg( $post_id, $field, $old, $new ) {$this->save_field( $post_id, $field, $old, $new );}
    
    /**
     * Save post meta repeater Fields.
     *
     * @param string $post_id 
     * @param string $field 
     ...
     * @access public 
     */
    public function save_post_meta_field_repeater( $post_id, $field, $old, $new ) {
     ...
     * @access public
     */
    public function save_post_meta_field_file( $post_id, $field, $old, $new ) {
     ...
     */
    public function HelpTab($args){$this->_help_tabs[] = $args;}
    ...
    /**
     * edit_insert_to_post_text rename insert to post button
     * 
     * @author  ohad raz
     * @since 0.1
    ...
    */
     ...
     * @access public
     */
    public function add_enctype () { echo ' enctype="multipart/form-data"';}
    
    /**
     * Handle_plupload_action handles plupload field uploads
     * @access public
     */
    ...
    public function show_field_repeater( $field, $meta ) {
      ...
      echo '<br/><style>.at-inline{line-height: 1 !important;}.at-inline .at-field{border: 0px !important;}.at-inline .at-label{margin: 0 0 1px !important;}.at-inline .at-text{width: 70px;}.at-inline .at-textarea{width: 100px; height: 75px;}  .at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}  </style>';
     ...
     * a template redirect hook which looks for that query var and if found calls the download function
     */
     ...
     * @return  boolean 
     */
    public function is_email($val){ return (bool)(preg_match("/^([a-z0-9+_-]+)(.[a-z0-9+_-]+)*@([a-z0-9-]+.)+[a-z]{2,6}$/ix",$val)); }
     ...
     * @return  boolean
     */
    public function is_numeric($val){ return (bool)preg_match('/^[-+]?[0-9]*.?[0-9]+$/', (int)$val);}
     ...
     * @return  boolean
     */
    public function is_minvalue($number,$max){ return (bool)((int)$number > (int)$max); }
     ...
     * @return  boolean
     */
    public function is_maxvalue($number,$max){ return ((int)$number < (int)$max); }
     ...
     * @return  boolean
     */
    public function is_minlength($val, $min){ return (strlen($val) >= (int)$min); }
     ...
     * @return  boolean
     */
    public function is_maxlength($val, $max){ return (strlen($val) <= (int)$max); }
     ...
     * @return  boolean
     */
    public function is_length($val, $length){ return (strlen($val) == (int)$length); }
     ...
     * @return  boolean
     */
    public function is_url($val){ return  (bool)preg_match('|^http(s)?://[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(/.*)?$|i', $val);}
     ...
     * @return  boolean
     */
    public function is_alphanumeric($val){ return (bool)preg_match("/^([a-zA-Z0-9])+$/i", $val); }
    ...
    public function load_textdomain(){
      //Works In themes/plugins/mu-plugins directory
